...
#     dp.add_handler(CommandHandler("start", bot_handlers.start), HandlerGroup.BASE)  # type: ignore
...
async def start_handler(event: events.NewMessage.Event):
    ...
    await event.respond("I am here")


class NerdDiaryTGBot(AsyncApplication):
    def __init__(
        self, *, config: NerdDiaryTGBotConfig = NerdDiaryTGBotConfig(), loop: asyncio.AbstractEventLoop = None
    ) -> None:
        super().__init__(loop=loop, logger=logger)
        self._conf = config

        bot = TelegramClient(None, int(config.API_ID.get_secret_value()), config.API_HASH.get_secret_value())
        self._bot = bot
    ...
    async def _astart(self):
        ...
        await self._bot.start(bot_token=self._conf.BOT_TOKEN.get_secret_value())
