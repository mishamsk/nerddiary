...
class User(BaseModel):
    ...
    def question_type_check_and_inject(cls, v: Poll, values: Dict[str, Any]):
        for question in v.questions.values():
            ...
            if question.type:
                ...
                custom_type_defs: Dict[str, QuestionType] = values.get("question_types", None)
                if custom_type_defs:
                ...
                if not q_type:
                    raise ValueError(f"Type definition for <{question.type}> is missing")
    ...
    @validator("polls")
    def validate_and_add_unique_question_selects_keys(cls, v: Dict[str, Poll], values: Dict[str, Any]):
            ...
            for name, question in poll.questions.items():
                ...
                if not question.depends_on and question.select:
                    ...
                    if poll.questions[question.depends_on].depends_on:  # type: ignore
                        ...
                        for sub_answer in poll.questions[question.depends_on].select.values():  # type: ignore
                        ...
                        for a_name, answer in poll.questions[question.depends_on].select.items():  # type: ignore
                            if answer.value not in question.select:  # type: ignore
                                raise ValueError(
                                    f"Question <{name}> depends on <{question.depends_on}> but doesn't have selects for answer <{a_name}> from dependant question"
                                )
                    ...
                    for sub_name, sub_select in question.select.items():
                        ...
                        for sel_name, sel_val in sub_select.items():  # type: ignore
                            new_key = str(next(callback_counter)) + "_" + sel_name
                            new_select[new_key] = ValueLabel(value=sel_name, label=sel_val.label)
                            if question.delay_on and sel_name in question.delay_on:
                    ...
                    if postponed_delete_delay_keys:
                        question.delay_on = list(set(question.delay_on) - postponed_delete_delay_keys)  # type: ignore
                    ...
                    if question.depends_on:
                        all_exist = all(
                            map(
                                ...
                                in list(
                                    itertools.chain(
                                        *[list(sub_a.keys()) for sub_a in question.select.values()]  # type: ignore
                                    )
                                ),
                                question.delay_on,
                            )
                        ...
                        all_exist = all(map(lambda x: x in question.select, question.delay_on))  # type: ignore
