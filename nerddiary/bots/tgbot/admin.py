...
def admin_handler_method(func):
    ...
    @wraps(func)
    def wrapped(self, update: Update, context: CallbackContext, *args, **kwargs) -> Optional[str]:
...
class BotAdmin:
    ...
    def __init__(
        ...
        if self.admin_list:
            for adm_chat_id in self.admin_list:
                self._add_admin_commands(adm_chat_id, dp, add_restart_command=add_restart_command)
    ...
    def _add_admin_commands(self, chat_id: int, dp: Dispatcher, add_restart_command: bool = False) -> None:
        commands = []
    ...
    def _check_user_invite(self, update: Update, context: CallbackContext) -> Optional[str]:
        ...
        if user_id not in allow_list:
            ...
            self._send_message_to_admin(context=context, text=text, reply_markup=keyboard)

            raise DispatcherHandlerStop()

    @admin_handler_method
    def _process_admin_user_action(self, update: Update, context: CallbackContext) -> Optional[str]:
    ...
    @admin_handler_method
    def _reload_handler(self, update: Update, context: CallbackContext) -> Optional[str]:
    ...
    def _check_ban_list(self, update: Update, context: CallbackContext) -> Optional[str]:
    ...
    def _check_allow_list(self, update: Update, context: CallbackContext) -> Optional[str]:
    ...
    def _send_message_to_admin(
        ...
        if self.admin_list:
            for admin in self.admin_list:
                context.bot.send_message(chat_id=admin, text=text, reply_markup=reply_markup)
