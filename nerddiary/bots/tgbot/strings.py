...
# bot.py
SERVER_ERROR = _(
    "🤯 Ой, что-то пошло не так. Кажется я сломался и меня нужно смазать. Я сообщу своему создателю, а ты попробуй позднее. Прошу прощения! 🙏"
)
CURRENT_VALUE_EMOJI = "✅"
START_REALOD_POLL_REMINDERS = _("Меня перезапускали. Восстанавливаю регулярные напоминания!")
...
USER_INVITE_APPROVED = _("Congrats! Your invitation request was approved by one of the admins. Please press /start")
USER_BANNED = _(
